...
export default function SchedulePage() {
  ...
  const { schedules, loading, fetchSchedules, triggerSchedule, reorderSchedules, clearDay } = useSchedule();
      ...
      <main className="page-content">
        ...
        <div className="page-header" style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            >
              {generating ? <><span className="spinner" /> กำลังจัดตาราง...</> : '🧠 จัดตารางอัตโนมัติ'}
            </button>
            <button
              type="button"
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: 20, alignItems: 'start' }}>
          ...
          <div className="glass" style={{ padding: '20px 24px' }}>
            <div style={{
              ...
              <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-secondary)' }}>
                {schedules.length > 0 ? `${schedules.length} ช่วงเวลา` : 'ไม่มีตาราง'}
              </div>
              {totalDuration > 0 && (
            ...
            {loading ? (
              ...
              <ScheduleTimeline schedules={schedules} onReorder={reorderSchedules} />
            )}
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {/* Energy gauge */}
            <div className="glass" style={{ padding: '20px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-secondary)', marginBottom: 16, alignSelf: 'flex-start' }}>
                ⚡ พลังงาน
              </div>
